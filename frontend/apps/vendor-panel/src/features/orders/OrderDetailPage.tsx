...
const OrderDetailPage: React.FC = () => {
    ...
    const handlePrintInvoice = () => {
        ...
        const html = `
            ...
            <head>
                <meta charset="UTF-8">
                <title>Fatura - #${order.orderNumber || order.id.slice(-6).toLowerCase()}</title>
                <style>
                    ...
                    .header h1 { margin: 0; font-size: 28px; color: #1e293b; font-style: ; font-weight: 900; }
                    .invoice-details { text-align: right; }
                    .info-section { display: flex; justify-content: space-between; margin-bottom: 40px; }
                    .info-box { background: #f8fafc; padding: 20px; border-radius: 8px; width: 45%; }
                    .info-box h3 { margin-top: 0; font-size: 14px; text-transform: ; color: #64748b; margin-bottom: 15px; }
                    table { width: 100%; border-collapse: collapse; margin-bottom: 40px; }
                    th { border-bottom: 2px solid #e2e8f0; padding: 12px 8px; text-align: left; font-size: 14px; text-transform: ; color: #64748b; }
            ...
            <body>
                <div class="header">
                    ...
                    <div class="invoice-details">
                        <strong style="font-size: 18px;">FATURA #${order.orderNumber || order.id.slice(-6).toLowerCase()}</strong>
                ...
                <div class="info-section">
                    <div class="info-box">
                        ...
                        <strong style="font-size: 18px;">${order.user?.name || 'Bilinmiyor'}</strong><br><br>
                        ${order.user?.email || 'Bilinmiyor'}<br>
                    </div>
                </div>
    ...
    if (isLoading) return (
        <div className="flex flex-col items-center justify-center py-40 gap-10 opacity-20">
            ...
            <span className="text-2xl font-semibold  ">YÜKLENİYOR...</span>
        </div>
    );
        ...
        <div className="space-y-6 md:space-y-10">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <button
                    ...
                    className="w-full md:w-auto px-6 md:px-12 py-4 md:py-6 bg-brand-pink text-white rounded-2xl text-xs md:text-sm font-semibold shadow-xl shadow-brand-pink/20 hover:bg-brand-pink-hover transition-all active:scale-95 flex items-center justify-center gap-4 "
                >
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 sm:gap-10">
                ...
                <div className="xl:col-span-2 space-y-6 sm:space-y-10">
                    <div className="bg-white rounded-xl shadow-sm border border-slate-50 overflow-hidden p-6 sm:p-10">
                        <h3 className="text-10px font-semibold text-slate-400    mb-5">Sipariş İçeriği</h3>
                        <div className="space-y-8">
                            {order.items.map((item, idx) => (
                                <div key={idx} className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-8 group">
                                    ...
                                    <div className="flex-1">
                                        <span className="text-lg sm:text-xl font-semibold text-slate-900 block leading-tight mb-2 group-hover:text-brand-pink transition-colors ">{item?.product?.name}</span>
                                        <div className="flex flex-wrap gap-4 items-center">
                                            <span className="text-10px font-semibold text-slate-400   bg-slate-50 px-3 py-1 rounded-lg ">ADET: {item?.quantity}</span>
                                            {item?.variantName && (
                                                <span className="text-10px font-semibold text-indigo-500   bg-indigo-50 px-3 py-1 rounded-lg ">{item?.variantName}: {item?.variantValue}</span>
                                            )}
                                        </div>
                                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-10 items-stretch">
                        <div className="bg-white rounded-xl p-6 sm:p-10 shadow-sm border border-slate-50 flex flex-col">
                            <h3 className="text-10px font-semibold text-slate-400    mb-4">Müşteri Bilgileri</h3>
                            <div className="space-y-6 flex-1 flex flex-col justify-center">
                                <div>
                                    <span className="text-nano font-semibold text-slate-300  block mb-1">İSİM SOYİSİM</span>
                                    <span className="text-lg font-semibold text-slate-900  ">{order.user?.name || 'Bilinmiyor'}</span>
                                </div>
                                <div>
                                    <span className="text-nano font-semibold text-slate-300  block mb-1">E-POSTA ADRESİ</span>
                                    <span className="text-lg font-semibold text-slate-900  ">{order.user?.email || 'Bilinmiyor'}</span>
                                </div>
                            </div>
                        </div>
                        <div className="bg-white rounded-xl p-6 sm:p-10 shadow-sm border border-slate-50 flex flex-col">
                            <h3 className="text-10px font-semibold text-slate-400    mb-4">Sipariş Durumu</h3>
                            <div className="space-y-6 flex-1 flex flex-col justify-center">
                                <div>
                                    <span className="text-nano font-semibold text-slate-300  block mb-1">GÜNCEL DURUM</span>
                                    <div className={`inline-flex px-6 py-3 rounded-2xl text-10px font-semibold  items-center gap-3   ${getStatusColor(order.status)} shadow-md`}>
                                ...
                                <div>
                                    ...
                                    <span className="text-lg font-semibold text-slate-900  ">{new Date(order.createdAt).toLocaleString('tr-TR')}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="space-y-6 sm:space-y-10">
                    <div className="bg-white rounded-xl p-8 sm:p-12 shadow-sm border border-slate-50 relative overflow-hidden group">
                        <div className="absolute top-[-20%] right-[-10%] w-64 h-64 bg-brand-pink/5 rounded-full blur-3xl pointer-events-none"></div>
                        <span className="text-slate-400 text-10px font-semibold   mb-5 block  relative z-10">Sipariş Özeti</span>
                        <div className="space-y-6 relative z-10">
                            <div className="flex justify-between items-center text-slate-400 font-semibold    text-10px">
                                <span>ARA TOPLAM</span>
                            ...
                            <div className="pt-8 border-t border-slate-50 flex justify-between items-end">
                                <span className="text-xs md:text-sm font-semibold  text-brand-pink mb-2">TOPLAM ÖDENEN</span>
                                <span className="text-2xl md:text-3xl font-semibold  text-slate-900 leading-none">{order.totalAmount.toLocaleString()} ₺</span>
                            </div>
                        </div>
                    </div>

                    <div className="bg-white rounded-xl p-6 sm:p-10 shadow-sm border border-slate-50">
                        <h3 className="text-10px font-semibold text-slate-400    mb-4">Hızlı Aksiyonlar</h3>
                        <div className="space-y-4">
                            <button onClick={handlePrintInvoice} className="w-full py-5 bg-slate-50 hover:bg-slate-100 rounded-2xl flex items-center justify-center gap-4 text-10px font-semibold   text-slate-600 transition-all  border border-transparent hover:border-slate-200 group">
