            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-12">
                <div className="xl:col-span-2 space-y-12">
                    ...
                    <div className="bg-white rounded-md p-12 shadow-sm border border-slate-50 relative overflow-hidden group">
                        <div className="absolute top-0 right-0 w-64 h-64 bg-slate-50 rounded-bl-[5rem] -translate-x-12 -translate-y-12 opacity-50 pointer-events-none group- transition-transform duration-1000"></div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                                ...
                                <div className="flex items-end">
                                    <button
                                        ...
                                        className="w-full h-12 bg-indigo-600 text-white rounded-md text-xs font-semibold  shadow-lg shadow-indigo-100  transition-all active:scale-95"
                                    >
                                        Seçenek Ekle
                                    </button>
                                </div>
                        ...
                        <div className="overflow-x-auto rounded-md border border-slate-100">
                            <table className="w-full text-left">
                                <thead className="bg-slate-50 border-b border-slate-100">
                                    <tr>
                                        <th className="px-6 py-4 text-10px font-semibold text-slate-400  ">Özellik</th>
                                        <th className="px-6 py-4 text-10px font-semibold text-slate-400  ">Değer</th>
                                        <th className="px-6 py-4 text-10px font-semibold text-slate-400  ">Fiyat</th>
                                        <th className="px-6 py-4 text-10px font-semibold text-slate-400  ">Stok</th>
                                        <th className="px-6 py-4 text-10px font-semibold text-slate-400   text-right">Sil</th>
                                    </tr>
                                </thead>
                                                ...
                                                {block.content && typeof block.content !== 'string' && block.content.url && (
                                                    <div className="flex bg-slate-100 p-1 rounded-xl gap-1">
                                                        <div className="flex border-r border-slate-200 pr-1 gap-1">
                                                            {['33', '50', '75', '100'].map(w => (
                                                                <button key={w} onClick={() => updateBlock(block.id, { ...block.content, width: w })} className={`px-2 py-1 text-nano font-semibold rounded-lg transition-all ${block.content.width === w ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>{w}%</button>
                                                            ))}
                                                        </div>
                                                        <div className="flex pl-1 gap-1">
                                                            {['left', 'center', 'right'].map(a => (
                                                                <button key={a} onClick={() => updateBlock(block.id, { ...block.content, align: a })} className={`px-2 py-1 text-nano font-semibold rounded-lg transition-all  ${block.content.align === a ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>{a[0]}</button>
                                                            ))}
                                                        </div>
                                                    </div>
                                                )}
                                                ...
                                                {block.content && (typeof block.content === 'string' ? block.content : block.content.url) ? (
                                                    <div className="relative group/img w-full">
                                                        ...
                                                        <label className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover/img:opacity-100 transition-all flex items-center justify-center cursor-pointer">
                                                            ...
                                                            <span className="text-white font-semibold text-10px   backdrop-blur-sm px-4 py-2 rounded-xl bg-white/10 border border-white/20">DEĞİŞTİR</span>
                                                        </label>
                                                    </div>
                                                ) : (
                                            ...
                                            <div key={comp.id} className="p-4 bg-white rounded-md border border-slate-100 hover:border-slate-200 transition-all space-y-4">
                                                ...
                                                {isChecked && (
                                                    <div className="flex items-center justify-between gap-3 pt-3 border-t border-slate-50 animate-in fade-in slide-in-from-top-2 duration-200">
                                                        <span className="text-10px font-semibold text-slate-400 r">EKSTRA ÜCRET</span>
