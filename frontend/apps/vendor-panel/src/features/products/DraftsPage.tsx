    ...
    return (
        <div className="space-y-12">
            <div className="flex justify-between items-end">
                <div>
                    <h1 className="text-[40px] font-[1000] text-slate-900 tracking-tighter mb-4 italic leading-none ">
            ...
            {drafts.length === 0 ? (
                <div className="bg-white rounded-xl p-20 text-center border-2 border-dashed border-slate-100">
                    ...
                    <Link to="/products/create" className="text-brand-pink font-semibold text-sm  tracking-widest hover:underline">Yeni Ürün Ekle</Link>
                </div>
            ) : (
                        ...
                        <div key={draft.id} className="group bg-white rounded-xl p-6 shadow-sm hover:shadow-xl hover:shadow-slate-200/50 transition-all border border-slate-50 relative">
                            <div className="aspect-square rounded-2xl bg-slate-50 overflow-hidden mb-6 relative">
                                <img
                                    ...
                                    alt={draft.name}
                                />
                                <div className="absolute top-4 right-4 bg-slate-900/80 backdrop-blur px-3 py-1 rounded-full text-8px font-semibold text-white  tracking-widest italic">TASLAK</div>
                            </div>
                            ...
                            <div className="flex justify-between items-center mb-6">
                                ...
                                <span className="text-9px font-bold text-slate-300  italic">{new Date(draft.createdAt).toLocaleDateString('tr-TR')}</span>
                            </div>

                            <div className="flex gap-3">
                                <button
                                    onClick={() => navigate(`/products/edit/${draft.id}`)}
                                    className="flex-1 h-16 bg-slate-50 rounded-2xl text-10px font-semibold text-slate-500 hover:bg-slate-900 hover:text-white transition-all  tracking-widest italic"
                                >
                                    DÜZENLE
                                </button>
                                <button
