    ...
    return (
        <div className="space-y-12">
            <div className="flex justify-between items-end">
                <div>
                    <h1 className="text-4xl font-[1000] text-slate-900  mb-4 italic leading-none ">
            ...
            {drafts.length === 0 ? (
                <div className="bg-white rounded-xl p-20 text-center border-2 border-dashed border-slate-100">
                    ...
                    <h3 className="text-2xl font-semibold text-slate-400 italic  mb-4">Taslak Ürün Bulunmuyor</h3>
                    <Link to="/products/create" className="text-brand-pink font-semibold text-sm   hover:underline">Yeni Ürün Ekle</Link>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                    {drafts.map((draft) => (
                        <div key={draft.id} className="group bg-white rounded-xl p-6 shadow-sm hover:shadow-xl hover:shadow-slate-200/50 transition-all border border-slate-50 relative">
                            <div className="aspect-square rounded-2xl bg-slate-50 overflow-hidden mb-6 relative">
                                <img
                                    ...
                                    className="w-full h-full object-contain group- transition-transform duration-700"
                                    alt={draft.name}
                                />
                                <div className="absolute top-4 right-4 bg-slate-900/80 backdrop-blur px-3 py-1 rounded-full text-micro font-semibold text-white   italic">TASLAK</div>
                            </div>

                            <h3 className="text-lg font-semibold text-slate-900 mb-2 truncate italic">{draft.name || 'İsimsiz Ürün'}</h3>
                            <div className="flex justify-between items-center mb-6">
                                <div className="flex items-baseline gap-2">
                                    {draft.discountPrice ? (
                                        <>
                                            ...
                                            <span className="text-brand-pink font-semibold text-xl italic">
                                ...
                                <span className="text-nano font-bold text-slate-300  italic">{new Date(draft.createdAt).toLocaleDateString('tr-TR')}</span>
                            </div>

                            <div className="flex gap-3">
                                <button
                                    onClick={() => navigate(`/products/edit/${draft.id}`)}
                                    className="flex-1 h-16 bg-slate-50 rounded-2xl text-10px font-semibold text-slate-500 hover:bg-slate-900 hover:text-white transition-all   italic"
                                >
                                    DÜZENLE
                                </button>
                                <button
