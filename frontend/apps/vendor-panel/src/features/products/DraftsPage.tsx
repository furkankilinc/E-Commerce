...
const DraftsPage: React.FC = () => {
    ...
    return (
        <div className="space-y-6 md:space-y-12">
            ...
            {drafts.length === 0 ? (
                <div className="bg-white rounded-xl p-10 sm:p-20 text-center border-2 border-dashed border-slate-100">
                    ...
                    <h3 className="text-xl sm:text-2xl font-semibold text-slate-400   mb-4">Taslak Ürün Bulunmuyor</h3>
                        ...
                        <div key={draft.id} className="group bg-white rounded-xl p-6 shadow-sm hover:shadow-xl hover:shadow-slate-200/50 transition-all border border-slate-50 relative">
                            <div className="aspect-square rounded-2xl bg-slate-50 overflow-hidden mb-6 relative">
                                <img
                                    ...
                                    alt={draft.name}
                                />
                                <div className="absolute top-4 right-4 bg-slate-900/80 backdrop-blur px-3 py-1 rounded-full text-micro font-semibold text-white   ">TASLAK</div>
                            </div>

                            <h3 className="text-lg font-semibold text-slate-900 mb-2 truncate ">{draft.name || 'İsimsiz Ürün'}</h3>
                            ...
                            <div className="flex items-center gap-3">
                                <button
                                    ...
                                    className="flex-1 h-12 bg-slate-50 rounded-2xl text-10px font-semibold text-slate-500 hover:bg-slate-900 hover:text-white transition-all   "
                                >
                                    DÜZENLE
                                </button>
                                <button
                                    onClick={(e) => handleDelete(e, draft.id)}
                                    className="w-12 h-12 bg-rose-50 rounded-2xl text-rose-500 hover:bg-rose-500 hover:text-white transition-all flex items-center justify-center shrink-0"
                                >
