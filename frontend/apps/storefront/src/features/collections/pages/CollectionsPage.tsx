...
const CollectionDetailView: React.FC<{ collectionId: string }> = ({ collectionId }) => {
    ...
    return (
        <div className="max-w-[1650px] mx-auto px-4 sm:px-10 lg:px-20 py-6 sm:py-10">
            {/* Back */}
            <Link to="/collections" className="inline-flex items-center gap-2 text-10px font-semibold text-gray-500   italic hover:text-brand-pink transition-colors mb-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-16">
                {isEditing ? (
                    ...
                    <div>
                        <p className="text-10px font-semibold text-brand-pink   italic mb-3">Koleksiyon</p>
                        <h1 className="text-3xl sm:text-4xl lg:text-4xl font-[1000] text-gray-900 leading-none  italic  flex items-center gap-4">
                            <span>{col.emoji}</span>
                            <span>{col.name}</span>
                        </h1>
                ...
                {!isEditing && (
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className="px-6 py-3 bg-gray-50 text-gray-500 rounded-md text-10px font-semibold   italic hover:bg-gray-100 transition-all"
                        >Düzenle</button>
                        <button
                            onClick={handleDelete}
                            className="px-6 py-3 bg-gray-50 text-red-400 rounded-md text-10px font-semibold   italic hover:bg-red-50 hover:text-red-500 transition-all"
                        >Sil</button>
                    </div>
                )}
            </div>
            ...
            {(col.items || []).length === 0 ? (
                <div className="flex flex-col items-center justify-center py-32 border-2 border-dashed border-gray-100 rounded-md bg-gray-50/30">
                    ...
                    <Link to="/shop" className="px-12 py-5 bg-brand-pink text-white rounded-md text-caption font-semibold   shadow-xl shadow-brand-pink/20  transition-all italic">
                        Ürünleri Keşfet
                    </Link>
                </div>
            ) : (
                    ...
                    {(col.items || []).map(item => (
                        <div key={item.id} className="group relative flex flex-col bg-white rounded-md p-4 transition-all border-2 border-transparent hover:shadow-xl hover:shadow-gray-200/50">
                            ...
                            <div className="flex flex-col flex-grow px-2">
                                {item.category && <span className="text-nano font-[1000] text-brand-pink   italic mb-2">{item.category}</span>}
                                <Link to={`/product/${item.id}`}>
                                    <h3 className="text-base font-[1000] text-gray-900 mb-6 leading-[1.2] group-hover:text-brand-pink transition-colors italic line-clamp-2  ">{item.name}</h3>
                                </Link>
            ...
            <div className="max-w-[1650px] mx-auto px-4 sm:px-10 lg:px-20 py-6 sm:py-10">
                {!isAuthenticated ? (
                    <div className="flex flex-col items-center justify-center py-32 border-2 border-dashed border-gray-100 rounded-md bg-gray-50/30">
                        ...
                        <p className="text-gray-500 text-sm font-bold mb-6 italic ">Koleksiyonlarınızı görmek ve yönetmek için lütfen oturum açın.</p>
                        <Link
                            to="/login"
                    ...
                    <>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-8 mb-8">
                            <div>
                                <p className="text-10px font-semibold text-brand-pink   italic mb-3">Listelerim</p>
                            ...
                            <button
                                ...
                                className="flex items-center gap-3 px-8 py-4 bg-gray-900 text-white rounded-md text-caption font-semibold   italic  hover:bg-brand-pink transition-all "
                            >
                        ...
                        {isCreating && (
                            <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6" onClick={(e) => { if (e.target === e.currentTarget) setIsCreating(false); }}>
                                ...
                                <div className="relative w-full max-w-md bg-white rounded-md shadow-xl z-10 p-10">
                                    <h2 className="text-2xl font-[1000] text-gray-900 italic   mb-4">Yeni Koleksiyon</h2>
                        ...
                        {(collections || []).length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-32 border-2 border-dashed border-gray-100 rounded-md bg-gray-50/30">
                                ...
                                <h2 className="text-3xl font-semibold text-gray-900 italic  mb-4 ">Henüz koleksiyon yok</h2>
                            ...
                            <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-8">
                                {(collections || []).map(col => (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-start justify-between">
                                            <div>
                                                <p className="text-xl font-[1000] text-gray-900 italic   group-hover:text-brand-pink transition-colors mb-1">
                                                    {col.emoji} {col.name}
                                                </p>
                                                <p className="text-10px font-semibold text-gray-400   italic">{(col.items || []).length} Ürün</p>
                                            </div>
                                ...
                                >
                                    ...
                                    <p className="text-caption font-semibold   italic">Yeni Koleksiyon</p>
                                </button>
                            </div>
                        )}
                    </>
