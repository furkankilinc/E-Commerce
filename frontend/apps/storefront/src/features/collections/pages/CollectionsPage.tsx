...
const CollectionDetailView: React.FC<{ collectionId: string }> = ({ collectionId }) => {
    ...
    if (!col) {
        return (
            <div className="text-center py-32">
                <p className="text-2xl font-semibold text-gray-900 italic  mb-4">Koleksiyon bulunamadı</p>
    ...
    return (
        <div className="max-w-[1440px] mx-auto px-6 sm:px-10 lg:px-20 py-10">
            {/* Back */}
            <Link to="/collections" className="inline-flex items-center gap-2 text-10px font-semibold text-gray-500  tracking-widest italic hover:text-brand-pink transition-colors mb-6">
                ...
                {isEditing ? (
                    <div className="flex-1 space-y-4">
                        ...
                        <input
                            ...
                            className="w-full max-w-md h-16 px-6 bg-gray-50 border-2 border-brand-pink rounded-md text-2xl font-semibold italic outline-none"
                            autoFocus
                        />
                        <div className="flex gap-3">
                            <button onClick={() => setIsEditing(false)} className="px-6 py-3 bg-gray-50 text-gray-500 rounded-md text-10px font-semibold  tracking-widest">İptal</button>
                            <button onClick={handleSaveRename} className="px-10 py-3 bg-brand-pink text-white rounded-md text-10px font-semibold  tracking-widest  shadow-brand-pink/20">Kaydet</button>
                        </div>
                    </div>
                ) : (
                    <div>
                        <p className="text-10px font-semibold text-brand-pink tracking-widest  italic mb-3">Koleksiyon</p>
                        <h1 className="text-3xl sm:text-4xl lg:text-[40px] font-[1000] text-gray-900 leading-none tracking-tighter italic  flex items-center gap-4">
                            <span>{col.emoji}</span>
                            <span>{col.name}</span>
                        </h1>
                ...
                {!isEditing && (
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className="px-6 py-3 bg-gray-50 text-gray-500 rounded-md text-10px font-semibold  tracking-widest italic hover:bg-gray-100 transition-all"
                        >Düzenle</button>
                        <button
                            onClick={handleDelete}
                            className="px-6 py-3 bg-gray-50 text-red-400 rounded-md text-10px font-semibold  tracking-widest italic hover:bg-red-50 hover:text-red-500 transition-all"
                        >Sil</button>
                    </div>
                )}
            </div>
            ...
            {col.items.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-32 border-2 border-dashed border-gray-100 rounded-md bg-gray-50/30">
                    <span className="text-[40px] mb-4">{col.emoji}</span>
                    <h2 className="text-2xl font-semibold text-gray-900 italic  mb-4">Bu koleksiyon boş</h2>
                    <p className="text-gray-500 text-sm font-bold mb-6 italic">Ürünleri ürün sayfasından koleksiyona ekleyebilirsiniz.</p>
                    <Link to="/shop" className="px-12 py-5 bg-brand-pink text-white rounded-md text-[11px] font-semibold  tracking-[0.25em] shadow-xl shadow-brand-pink/20 hover:scale-105 transition-all italic">
                        Ürünleri Keşfet
                    </Link>
                </div>
            ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-x-8 gap-y-12">
                    {col.items.map(item => (
                        <div key={item.id} className="group relative flex flex-col bg-white rounded-md p-6 border-2 border-transparent hover:border-gray-50 hover:shadow-xl hover:shadow-gray-200/50 transition-all">
                            <button
                            ...
                            <div className="flex flex-col flex-grow px-2">
                                {item.category && <span className="text-9px font-[1000] text-brand-pink tracking-widest  italic mb-2">{item.category}</span>}
                                <Link to={`/product/${item.id}`}>
                                    <h3 className="text-base font-[1000] text-gray-900 mb-6 leading-[1.2] group-hover:text-brand-pink transition-colors italic line-clamp-2  tracking-tighter">{item.name}</h3>
                                </Link>
                                <div className="mt-auto flex justify-between items-center pt-6 border-t border-gray-50">
                                    ...
                                    <button
                                        ...
                                        className="w-14 h-14 rounded-md bg-gray-900 text-white flex items-center justify-center hover:bg-brand-pink transition-all transform hover:scale-110 shadow-xl shadow-gray-200"
                                    >
            ...
            <div className="max-w-[1440px] mx-auto px-6 sm:px-10 lg:px-20 py-10">
                {!isAuthenticated ? (
                    <div className="flex flex-col items-center justify-center py-32 border-2 border-dashed border-gray-100 rounded-md bg-gray-50/30">
                        ...
                        <h2 className="text-3xl font-semibold text-gray-900 mb-4  tracking-tighter italic">Giriş Yapmalısınız</h2>
                        <p className="text-gray-500 text-sm font-bold mb-6 italic tracking-wide">Koleksiyonlarınızı görmek ve yönetmek için lütfen oturum açın.</p>
                        <Link
                            to="/login"
                            className="px-12 py-5 bg-brand-pink text-white rounded-md text-[11px] font-semibold  tracking-[0.25em] shadow-xl shadow-brand-pink/20 hover:scale-105 transition-all italic"
                        >
                            Giriş Yap
                        </Link>
                    </div>
                    ...
                    <>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-8 mb-8">
                            <div>
                                <p className="text-10px font-semibold text-brand-pink tracking-widest  italic mb-3">Listelerim</p>
                                <h1 className="text-3xl sm:text-4xl lg:text-[40px] font-[1000] text-gray-900 leading-none tracking-tighter italic ">
                                    KOLEKSİ<span className="text-brand-pink">YONLARIM</span>
                                </h1>
                            </div>
                            <button
                                onClick={() => setIsCreating(true)}
                                className="flex items-center gap-3 px-8 py-4 bg-gray-900 text-white rounded-md text-[11px] font-semibold  tracking-widest italic  hover:bg-brand-pink transition-all hover:scale-105"
                            >
                        ...
                        {isCreating && (
                            <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6" onClick={(e) => { if (e.target === e.currentTarget) setIsCreating(false); }}>
                                ...
                                <div className="relative w-full max-w-md bg-white rounded-md shadow-xl z-10 p-10">
                                    <h2 className="text-2xl font-[1000] text-gray-900 italic  tracking-tight mb-4">Yeni Koleksiyon</h2>
                                    ...
                                    <input
                                        ...
                                        className="w-full h-16 px-6 bg-gray-50 border-2 border-transparent focus:border-brand-pink rounded-md text-sm font-semibold italic outline-none transition-all placeholder:text-gray-300 mb-6"
                                    />
                                    <div className="flex gap-4">
                                        <button onClick={() => setIsCreating(false)} className="flex-1 h-14 rounded-md bg-gray-50 text-gray-500 text-10px font-semibold  tracking-widest">İptal</button>
                                        <button
                                            onClick={handleCreate}
                                            disabled={!newName.trim() || isSubmitting}
                                            className="flex-1 h-14 rounded-md bg-brand-pink text-white text-10px font-semibold  tracking-widest  shadow-brand-pink/20 hover:scale-[1.02] transition-all disabled:opacity-50"
                        ...
                        {collections.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-32 border-2 border-dashed border-gray-100 rounded-md bg-gray-50/30">
                                ...
                                <h2 className="text-3xl font-semibold text-gray-900 italic  mb-4 tracking-tighter">Henüz koleksiyon yok</h2>
                                <p className="text-gray-500 text-sm font-bold mb-6 italic">Ürünleri kategorilere göre organize et.</p>
                                <button
                                    onClick={() => setIsCreating(true)}
                                    className="px-12 py-5 bg-brand-pink text-white rounded-md text-[11px] font-semibold  tracking-[0.25em] shadow-xl shadow-brand-pink/20 hover:scale-105 transition-all italic"
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-8">
                                {collections.map(col => (
                                    <Link
                                        ...
                                        className="group flex flex-col bg-white rounded-md p-8 border-2 border-transparent hover:border-gray-100 hover:shadow-xl hover:shadow-gray-200/50 transition-all"
                                    >
                                        {/* Cover images */}
                                        ...
                                        <div className="flex items-start justify-between">
                                            <div>
                                                <p className="text-xl font-[1000] text-gray-900 italic  tracking-tight group-hover:text-brand-pink transition-colors mb-1">
                                                    {col.emoji} {col.name}
                                                </p>
                                                <p className="text-10px font-semibold text-gray-400  tracking-widest italic">{col.items.length} Ürün</p>
                                            </div>
                                ...
                                >
                                    ...
                                    <p className="text-[11px] font-semibold  tracking-widest italic">Yeni Koleksiyon</p>
                                </button>
                            </div>
                        )}
                    </>
