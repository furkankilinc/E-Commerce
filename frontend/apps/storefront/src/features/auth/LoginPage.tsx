        ...
        <div className="min-h-screen w-full flex flex-col bg-[#fafafa] relative overflow-x-hidden font-sans selection:bg-brand-pink/10 selection:text-brand-pink">
            ...
            <header className="fixed top-0 left-0 right-0 h-16 md:h-16 px-6 md:px-12 flex items-center justify-between bg-white/60 backdrop-blur-2xl border-b border-gray-100/50 z-50 transition-all">
                <div className="flex items-center gap-2 group cursor-pointer">
                    <span className="text-xl md:text-2xl font-[1000] tracking-tighter text-gray-900  italic leading-none">
                ...
                <nav className="hidden lg:flex items-center gap-8">
                    <a href="#" className="text-[11px] font-bold text-gray-400 hover:text-gray-900 transition-colors  tracking-[0.2em]">Mağaza</a>
                    <a href="#" className="text-[11px] font-bold text-gray-400 hover:text-gray-900 transition-colors  tracking-[0.2em]">Topluluk</a>
                    <a href="#" className="text-[11px] font-bold text-gray-400 hover:text-gray-900 transition-colors  tracking-[0.2em]">Yenilikler</a>
                    <Link to="/register" className="px-6 py-2 rounded-full border-2 border-gray-900 text-[11px] font-[1000] text-gray-900 hover:bg-gray-900 hover:text-white transition-all transform active:scale-95  tracking-widest shadow-sm">
                        Üye Ol
                    </Link>
                </nav>
            ...
            <div className={`fixed inset-0 bg-white z-[55] lg:hidden transition-all duration-500 ease-in-out flex flex-col ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                <div className="h-16 px-6 flex items-center justify-between border-b border-gray-50">
                    <span className="text-xl font-[1000] tracking-tighter text-gray-900  italic">FUI<span className="text-brand-pink">RA</span></span>
                ...
                <div className="flex-1 overflow-y-auto py-6 px-6 space-y-4">
                    ...
                    <button className="w-full text-left py-4 text-xl font-[1000] text-gray-900  italic border-b border-gray-50 flex justify-between">YENİLİKLER <span className="bg-brand-pink text-white text-9px px-2 py-0.5 rounded-full">HOT</span></button>
                </div>
            ...
            <footer className="w-full py-8 px-6 md:px-12 bg-white/40 backdrop-blur-md border-t border-gray-100/50 hidden md:flex flex-row justify-between items-center gap-6 z-20">
                <div className="flex flex-col items-start gap-1">
                    <span className="text-sm font-[1000] text-gray-900  tracking-tighter italic">FUI<span className="text-brand-pink">RA</span></span>
                ...
                <div className="flex items-center gap-8 md:gap-10">
                    {['Instagram', 'Twitter', 'Discord'].map(link => (
                        <a key={link} href="#" className="text-10px font-semibold text-gray-400 hover:text-gray-900  tracking-widest transition-colors">{link}</a>
                    ))}
                </div>
            </footer>
        </div>
