...
const renderBrandLogo = (brand: CardInfo['brand']) => {
    switch (brand) {
        case 'Visa':
            return (
                <span className="text-[#1434CB] font-[1000] italic text-12px tracking-tighter leading-none flex items-center select-none">
                    VISA
                </span>
            );
        case 'MasterCard':
        ...
        case 'Amex':
            return (
                <div className="px-1.5 py-0.5 bg-[#007BC1] rounded text-8px font-semibold text-white tracking-widest leading-none flex items-center justify-center h-4 select-none shrink-0">
                    AMEX
                </div>
            );
        case 'Troy':
            return (
                <div className="px-1.5 py-0.5 bg-[#002C6C] rounded text-8px font-semibold text-white italic tracking-tighter leading-none flex items-center justify-center gap-0.5 h-4 select-none shrink-0">
                    <span className="text-[#FF5C39]">tr</span><span>oy</span>
                </div>
            );
        default:
            return brand ? (
                <span className="text-slate-800 font-[1000] italic text-9px uppercase tracking-wider select-none shrink-0">
                    {brand}
                </span>
            ) : null;
    }
            ...
            <div className="bg-white rounded-lg shadow-xl border border-gray-100 w-full max-w-7xl h-[75vh] flex flex-col relative overflow-hidden animate-in zoom-in-95 duration-300">
                ...
                <div className="flex justify-between items-center px-8 py-6 border-b border-gray-100 bg-slate-50/50">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-xl font-[1000] text-gray-900 italic tracking-tighter">KART <span className="text-brand-pink">DEFTERİM</span></h3>
                    </div>
                    <button
                        onClick={() => {
                            onClose();
                ...
                <div className="flex-1 grid grid-cols-1 lg:grid-cols-5 overflow-hidden text-slate-800">
                    ...
                    <div className="lg:col-span-2 overflow-y-auto p-6 border-r border-gray-100 bg-gray-50/30 flex flex-col justify-between">
                        <div className="space-y-4">
                            <span className="text-9px font-semibold text-gray-400 tracking-widest block mb-4 italic">KAYITLI KARTLARINIZ</span>
                                        ...
                                        >
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                                                    <span className="text-9px font-semibold tracking-widest text-brand-pink uppercase italic truncate max-w-[100px]">{card.title}</span>
                                                    <div className="px-1.5 py-0.5 bg-slate-50 border border-slate-100 rounded shrink-0">
                                                        {renderBrandLogo(card.brand)}
                                                    </div>
                                                    <span className="text-10px font-semibold tracking-wider text-slate-700 italic uppercase truncate max-w-[80px]">{card.bank}</span>
                                                    {isSelected && <span className="text-emerald-500 text-8px font-semibold italic tracking-widest ml-auto">AKTİF</span>}
                                                </div>

                                                <h4 className="text-[11px] font-semibold text-gray-900 mb-0.5 tracking-wider font-mono">
                                                    •••• •••• •••• {card.number.replace(/\D/g, '').slice(-4)}
                                                </h4>
                                                <div className="flex justify-between items-center pr-2 mt-1">
                                                    <p className="text-9px font-bold text-gray-400 italic truncate max-w-[120px]">{card.holder}</p>
                                                    <p className="text-9px font-bold text-slate-500 font-mono shrink-0">{card.expiry}</p>
                                                </div>
                                            </div>
                        ...
                        <button
                            ...
                            className="w-full mt-6 py-4 border border-dashed border-gray-200 hover:border-brand-pink text-gray-400 hover:text-brand-pink rounded-md text-10px font-semibold tracking-widest transition-all italic flex items-center justify-center gap-2 cursor-pointer bg-white"
                        >
                            ＋ YENİ KART FORMU AÇ
                        </button>
                    </div>
                    ...
                    <div className="lg:col-span-3 overflow-y-auto p-8 flex flex-col justify-between">
                        <div className="space-y-6">
                            ...
                            <div className="space-y-4 text-slate-800">
                                <div className="space-y-1.5">
                                    <label className="text-9px font-semibold text-gray-400 tracking-widest ml-2 italic">KART ETİKETİ (Örn: Maaş Kartım, Bonus) <span className="text-brand-pink">*</span></label>
                                    <input
                                        type="text"
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-9px font-semibold text-gray-400 tracking-widest ml-2 italic">KART NUMARASI <span className="text-brand-pink">*</span></label>
                                    <input
                                        type="text"
                                        maxLength={19}
                                        value={newCardForm.number}
                                        onChange={(e) => setNewCardForm({ ...newCardForm, number: e.target.value.replace(/\D/g, '').replace(/(.{4})/g, '$1 ').trim() })}
                                        className="w-full h-10 bg-gray-50 border border-gray-100 rounded-md px-6 text-xs font-bold focus:outline-none focus:border-brand-pink focus:bg-white transition-all italic text-slate-800 font-mono tracking-wider"
                                        placeholder="4242 4242 4242 4242"
                                    />
                                </div>

                                <div className="space-y-1.5">
                                    <label className="text-9px font-semibold text-gray-400 tracking-widest ml-2 italic">KART ÜZERİNDEKİ İSİM <span className="text-brand-pink">*</span></label>
                                    <input
                                        type="text"
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1.5">
                                        <label className="text-9px font-semibold text-gray-400 tracking-widest ml-2 italic">SON KULLANMA TARİHİ <span className="text-brand-pink">*</span></label>
                                        <input
                                            type="text"
                                            maxLength={5}
                                    ...
                                    <div className="space-y-1.5">
                                        <label className="text-9px font-semibold text-gray-400 tracking-widest ml-2 italic">GÜVENLİK KODU (CVV) <span className="text-brand-pink">*</span></label>
                                        <input
                                            type="password"
                                            maxLength={3}
                        ...
                        <button
                            ...
                            className="w-full mt-8 py-5 bg-brand-pink text-white rounded-md text-10px font-semibold tracking-widest shadow-xs cursor-pointer flex items-center justify-center gap-2 hover:bg-rose-600 transition-colors italic"
                        >
