    ...
    if (items.length === 0) {
        return (
            <div className="min-h-[80vh] flex flex-col items-center justify-center px-4">
                ...
                <h1 className="text-4xl font-[1000] text-gray-900  italic  mb-4 leading-none">SEPETİNİZ <span className="text-brand-pink">BOŞ</span></h1>
                <p className="text-gray-400 font-bold   text-xs mb-5 italic">Henüz sepetinize bir ürün eklemediniz.</p>
                <Link to="/shop" className="px-12 py-5 bg-gray-900 text-white rounded-md text-caption font-semibold   hover:bg-brand-pink  transition-all  shadow-gray-200 italic">ALIŞVERİŞE BAŞLA</Link>
            </div>
        );
    }
    ...
    return (
        <div className="max-w-[1650px] mx-auto px-4 sm:px-10 lg:px-20 py-6 sm:py-10">
            <div className="mb-8">
                ...
                <h1 className="text-4xl font-[1000] text-gray-900  italic  leading-none mb-4">SEPETİM <span className="text-brand-pink">({itemCount})</span></h1>
                <p className="text-gray-400 font-bold   text-10px italic">Siparişinizi tamamlamadan önce ürünlerinizi gözden geçirin.</p>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 lg:gap-16 xl:gap-20">
                        ...
                        <div key={`${item.id}-${item.variant}`} className="flex flex-col md:grid grid-cols-6 items-center gap-6 group pb-8 md:pb-0 border-b border-gray-100/50 md:border-none">
                            <div className="col-span-3 flex items-center gap-4 sm:gap-8 w-full md:w-auto">
                                ...
                                <div className="min-w-0 flex-1">
                                    <h3 className="text-base sm:text-lg font-semibold text-gray-900 italic truncate max-w-[200px] sm:max-w-[300px] leading-tight mb-1 sm:mb-2">{item.name}</h3>
                                    <p className="text-10px font-bold text-gray-400 italic">{item.variant || 'Standard Edition'}</p>

                                    {/* Mobile-only Price Display */}
                                    <div className="md:hidden mt-2 flex items-baseline gap-2">
                                        {item.originalPrice && (
                                            <span className="text-[10px] text-gray-400 line-through italic">{item.originalPrice.toLocaleString()} ₺</span>
                                        )}
                                        <span className="text-xs font-extrabold text-brand-pink italic">{item.price.toLocaleString()} ₺</span>
                                    </div>

                                    <button
                                        onClick={() => removeItem(item.id, item.variant)}
                                        className="mt-3 sm:mt-4 text-nano font-semibold text-red-400 hover:text-red-500 italic flex items-center gap-2 transition-colors"
                                    >
                            ...
                            <div className="text-right hidden md:block">
                                {item.originalPrice && (
                                    <div className="text-micro font-semibold text-brand-pink italic mb-1">
                                        İNDİRİMLİ FİYAT
                                    </div>
                                )}
                            ...
                            <div className="text-center md:text-right mt-4 md:mt-0 flex justify-between md:flex-col items-center md:items-end w-full md:w-auto">
                                <span className="text-xs font-bold text-gray-400 md:hidden italic">TOPLAM FİYAT</span>
                ...
                <div className="col-span-1">
                    <div className="bg-gray-50/50 p-12 rounded-md border border-gray-100 sticky top-32">
                        <h2 className="text-2xl font-[1000] text-gray-900  italic  mb-5 leading-none">SİPARİŞ <span className="text-brand-pink">ÖZETİ</span></h2>
                        ...
                        <div className="space-y-6 pb-10 border-b border-gray-100">
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <span className="text-10px font-semibold text-emerald-500   italic">ÜCRETSİZ</span>
                            </div>
                            <div className="flex justify-between items-center">
                        ...
                        <div className="pt-10 mb-6 flex justify-between items-end">
                            <div>
                                ...
                                <span className="text-4xl font-[1000] text-gray-900 leading-none  italic whitespace-nowrap">{(total + (total * 0.2)).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₺</span>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            <button
                                ...
                                className="w-full py-3.5 bg-brand-pink text-white rounded-md text-caption font-semibold   shadow-xl shadow-brand-pink/20  hover:bg-brand-pink-hover transition-all italic flex items-center justify-center gap-4 group"
                            >
                                ÖDEMEYE GEÇ
                            ...
                            <Link to="/shop" className="w-full py-4 bg-white text-gray-900 border border-gray-100 rounded-md text-10px font-semibold   flex items-center justify-center hover:bg-gray-900 hover:text-white transition-all italic">ALIŞVERİŞE DEVAM ET</Link>
                        </div>

                        {/* Promo Code */}
                        <div className="mt-12">
                            <label className="text-nano font-semibold text-gray-300   mb-4 block italic ml-4">PROMOSYON KODU</label>
                            <div className="relative group">
                                <input type="text" placeholder="KOD GİRİNİZ" className="w-full h-14 bg-white border border-gray-100 rounded-md px-6 text-10px font-semibold focus:outline-none focus:ring-4 focus:ring-brand-pink/5 focus:border-brand-pink transition-all italic  " />
                                <button className="absolute right-2 top-2 bottom-2 px-6 bg-gray-50 hover:bg-gray-100 text-gray-900 rounded-md text-nano font-semibold   transition-all italic">UYGULA</button>
                            </div>
                        </div>
                    </div>
                </div>
