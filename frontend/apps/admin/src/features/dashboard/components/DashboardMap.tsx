...
const DashboardMap: React.FC<DashboardMapProps> = ({ points }) => {
    ...
    return (
        <div className="relative w-full h-[600px] rounded-xl overflow-hidden border border-slate-100 shadow-xl bg-slate-50">
            <div ref={mapRef} className="w-full h-full z-0" />

            <div className="absolute top-8 left-8 z-[10] bg-white/90 backdrop-blur-xl p-5 rounded-2xl border border-white shadow-xl pointer-events-none">
                <div className="space-y-3">
                    <div className="flex items-center gap-3">
                        <div className="w-3.5 h-3.5 rounded-full bg-indigo-500 shadow-lg shadow-indigo-500/30 border-2 border-white"></div>
                        <span className="text-10px font-semibold text-slate-700  tracking-widest italic">Satıcılar</span>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className="w-3.5 h-3.5 rounded-full bg-pink-500 shadow-lg shadow-pink-500/30 border-2 border-white"></div>
                        <span className="text-10px font-semibold text-slate-700  tracking-widest italic">Aktif Kullanıcılar</span>
                    </div>
                </div>
            </div>
            ...
            {!window.google && (
                <div className="absolute inset-0 flex items-center justify-center bg-slate-50/80 backdrop-blur-sm z-[20]">
                    <div className="text-center">
                        ...
                        <p className="text-sm font-semibold text-slate-400  tracking-widest italic">Google Maps Yükleniyor...</p>
                    </div>
                </div>
            )}
        </div>
